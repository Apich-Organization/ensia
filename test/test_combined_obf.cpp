...
uint64_t test_math_algebra(uint64_t a, uint64_t b, uint64_t c) {
...
__attribute__((noinline))
uint32_t test_aes_sbox_hash(uint32_t seed, int rounds) {
    uint32_t state = seed;
...
__attribute__((noinline))
uint32_t test_ackermann(uint32_t m, uint32_t n) {
    if (m == 0) return n + 1;
    if (m > 0 && n == 0) return test_ackermann(m - 1, 1);
    return test_ackermann(m - 1, test_ackermann(m, n - 1));
}

// 4. Matrix Multiplication (Vector & SIMD test)
__attribute__((noinline))
void test_matrix_mul(const float* A, const float* B, float* C, int N) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            float sum = 0.0f;
            for (int k = 0; k < N; k++) {
...
__attribute__((noinline))
std::string test_string_format(const std::string& input, int magic) {
...
int main() {
    std::cout << "[Ensia Combined Test] Running Automated Verification..." << std::endl;

    // Test 1: Math & Algebra
    uint64_t m_res = test_math_algebra(0x12345678ULL, 0x87654321ULL, 0x99887766ULL);
    uint64_t m_expected = test_math_algebra(0x12345678ULL, 0x87654321ULL, 0x99887766ULL);
    assert(m_res == m_expected);
    std::cout << "  [PASS] Test 1: Math & Algebra (Result: 0x" << std::hex << m_res << std::dec << ")" << std::endl;

    // Test 2: AES S-Box Hash
    uint32_t aes_res = test_aes_sbox_hash(0xDEADBEEFU, 20);
    std::cout << "  [PASS] Test 2: AES S-Box (Result: 0x" << std::hex << aes_res << std::dec << ")" << std::endl;
    ...
    std::string expected_str = "ENSIA_SECRET_PREFIX_2026_TEST_PAYLOAD_1757_PROTECTED_BY_OLLVM_NEXT";
    assert(str_res == expected_str);
    std::cout << "  [PASS] Test 5: String Encryption ('" << str_res << "')" << std::endl;

    std::cout << "[SUCCESS] ALL OBFUSCATION INTEGRATION TESTS PASSED 100%!" << std::endl;
    return 0;
}
