...
int main(void) {
  ...
  if (err != NO_ERROR) {
    test_print_result("AES_GCM", 0, NULL, 0);
    return 1;
  }
  ...
  TEST_MARK_START();
  err = gcmEncrypt(&gcmCtx, TV_NONCE_12, sizeof(TV_NONCE_12), TV_AAD,
                   sizeof(TV_AAD), TV_MSG_32, g_ct, sizeof(TV_MSG_32), g_tag,
                   sizeof(g_tag));
  TEST_MARK_END();
  ...
  err = gcmDecrypt(&gcmCtx, TV_NONCE_12, sizeof(TV_NONCE_12), TV_AAD,
                   sizeof(TV_AAD), g_ct, g_pt, sizeof(TV_MSG_32), g_tag,
                   sizeof(g_tag));
