...
int main(void) {
  ...
  err = chacha20Poly1305Encrypt(TV_CHACHA_KEY, sizeof(TV_CHACHA_KEY),
                                TV_NONCE_12, sizeof(TV_NONCE_12), TV_AAD,
                                sizeof(TV_AAD), TV_MSG_32, g_ct,
                                sizeof(TV_MSG_32), g_tag, sizeof(g_tag));
  TEST_MARK_END();

  if (err != NO_ERROR) {
    test_print_result("CHACHA20_POLY1305", 0, NULL, 0);
    return 1;
  }

  err =
      chacha20Poly1305Decrypt(TV_CHACHA_KEY, sizeof(TV_CHACHA_KEY), TV_NONCE_12,
                              sizeof(TV_NONCE_12), TV_AAD, sizeof(TV_AAD), g_ct,
                              g_pt, sizeof(TV_MSG_32), g_tag, sizeof(g_tag));
