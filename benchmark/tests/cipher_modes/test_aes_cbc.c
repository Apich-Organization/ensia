...
int main(void) {
  ...
  if (err != NO_ERROR) {
    test_print_result("AES_CBC", 0, NULL, 0);
    return 1;
  }

  TEST_MARK_START();
  err = cbcEncrypt(AES_CIPHER_ALGO, &aesCtx, s_iv_enc, TV_MSG_32, g_ct,
                   sizeof(TV_MSG_32));
  TEST_MARK_END();
  ...
  err = cbcDecrypt(AES_CIPHER_ALGO, &aesCtx, s_iv_dec, g_ct, g_pt,
                   sizeof(TV_MSG_32));
  aesDeinit(&aesCtx);
