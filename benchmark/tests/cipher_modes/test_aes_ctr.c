...
int main(void) {
  ...
  if (err != NO_ERROR) {
    test_print_result("AES_CTR", 0, NULL, 0);
    return 1;
  }

  TEST_MARK_START();
  err = ctrEncrypt(AES_CIPHER_ALGO, &aesCtx, 32, g_ctr_enc, TV_MSG_32, g_ct,
                   sizeof(TV_MSG_32));
  TEST_MARK_END();
  ...
  err = ctrEncrypt(AES_CIPHER_ALGO, &aesCtx, 32, g_ctr_dec, g_ct, g_pt,
                   sizeof(TV_MSG_32));
  aesDeinit(&aesCtx);
