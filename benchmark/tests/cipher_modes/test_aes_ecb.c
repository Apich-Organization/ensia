...
int main(void) {
  ...
  if (err != NO_ERROR) {
    test_print_result("AES_ECB_MODE", 0, NULL, 0);
    return 1;
  }

  TEST_MARK_START();
  err =
      ecbEncrypt(AES_CIPHER_ALGO, &aesCtx, TV_MSG_32, g_ct, sizeof(TV_MSG_32));
  TEST_MARK_END();
  ...
  err = ecbDecrypt(AES_CIPHER_ALGO, &aesCtx, g_ct, g_pt, sizeof(TV_MSG_32));
  aesDeinit(&aesCtx);
