...
static const uint8_t s_privkey[ED25519_PRIVATE_KEY_LEN] = {
    ...
    0xf4, 0x92, 0xec, 0x2c, 0x44, 0x34, 0x86, 0x15, 0x97, 0x18, 0x8b,
    0xf8, 0x6d, 0xbf, 0x3d, 0x90, 0xf0, 0x21, 0x76, 0x96, 0x3d};
...
int main(void) {
  ...
  if (err != NO_ERROR) {
    test_print_result("ED25519", 0, NULL, 0);
    return 1;
  }

  TEST_MARK_START();
  err = ed25519GenerateSignature(s_privkey, g_pubkey, TV_MSG_32,
                                 sizeof(TV_MSG_32), NULL, 0, 0, g_sig);
  if (err == NO_ERROR) {
    err = ed25519VerifySignature(g_pubkey, TV_MSG_32, sizeof(TV_MSG_32), NULL,
                                 0, 0, g_sig);
  }
  TEST_MARK_END();
