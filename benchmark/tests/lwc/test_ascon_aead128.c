...
int main(void)
{
    ...
    TEST_MARK_START();
    err = asconAead128Encrypt(TV_ASCON_KEY, sizeof(TV_ASCON_KEY),
                              TV_ASCON_NONCE, sizeof(TV_ASCON_NONCE),
                              TV_AAD, sizeof(TV_AAD),
                              TV_MSG_16, g_ct, sizeof(TV_MSG_16),
                              g_tag, sizeof(g_tag));
    TEST_MARK_END();

    if (err != NO_ERROR) { test_print_result("ASCON_AEAD128", 0, NULL, 0); return 1; }

    err = asconAead128Decrypt(TV_ASCON_KEY, sizeof(TV_ASCON_KEY),
                              TV_ASCON_NONCE, sizeof(TV_ASCON_NONCE),
                              TV_AAD, sizeof(TV_AAD),
                              g_ct, g_pt, sizeof(TV_MSG_16),
                              g_tag, sizeof(g_tag));
