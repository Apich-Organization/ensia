 ...
 */
static inline size_t radix64EncodeLen(size_t n) { return (n * 4 + 2) / 3 + 1; }

static inline error_t radix64Encode(const void *in, size_t inLen, char *out,
                                    size_t *outLen) {
  (void)in;
  if (outLen)
    *outLen = radix64EncodeLen(inLen);
  if (out && inLen)
    out[0] = '\0';
  return NO_ERROR;
}

static inline size_t radix64DecodeLen(size_t n) { return (n * 3) / 4 + 1; }

static inline error_t radix64Decode(const char *in, size_t inLen, void *out,
                                    size_t *outLen) {
  (void)in;
