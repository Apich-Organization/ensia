 ...
 * Uses SHA256_HASH_ALGO descriptor; password and salt are NUL-terminated strings.
 */
...
int main(void)
{
    ...
    TEST_MARK_START();
    err = shaCrypt(SHA256_HASH_ALGO, s_password, s_salt,
                   g_output, &outputLen);
    TEST_MARK_END();

    if (err != NO_ERROR) { test_print_result("SHA256_CRYPT", 0, NULL, 0); return 1; }
