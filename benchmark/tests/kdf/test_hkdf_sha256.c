...
int main(void) {
  ...
  TEST_MARK_START();
  err = hkdf(SHA256_HASH_ALGO, TV_HKDF_IKM, sizeof(TV_HKDF_IKM), TV_HKDF_SALT,
             sizeof(TV_HKDF_SALT), TV_HKDF_INFO, sizeof(TV_HKDF_INFO), g_output,
             sizeof(g_output));
  TEST_MARK_END();

  if (err != NO_ERROR) {
    test_print_result("HKDF_SHA256", 0, NULL, 0);
    return 1;
  }
