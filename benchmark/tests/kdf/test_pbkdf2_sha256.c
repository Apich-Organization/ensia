...
int main(void) {
  ...
  TEST_MARK_START();
  err = pbkdf2(SHA256_HASH_ALGO, TV_HMAC_KEY_32, sizeof(TV_HMAC_KEY_32),
               TV_HKDF_SALT, sizeof(TV_HKDF_SALT), TV_PBKDF2_ITERATIONS,
               g_output, sizeof(g_output));
  TEST_MARK_END();

  if (err != NO_ERROR) {
    test_print_result("PBKDF2_SHA256", 0, NULL, 0);
    return 1;
  }
