...
static const Ed25519Point ED25519_B = {
    {0x0F25D51A, 0x0AB16B04, 0x0969ECB2, 0x198EC12A, 0x0DC5C692, 0x1118FEEB,
     0x0FFB0293, 0x1A79ADCA, 0x00216936},
    {0x06666658, 0x13333333, 0x19999999, 0x0CCCCCCC, 0x06666666, 0x13333333,
     0x19999999, 0x0CCCCCCC, 0x00666666},
    {0x00000001, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000,
     0x00000000, 0x00000000, 0x00000000},
    {0x05B7DDA3, 0x0EF4559D, 0x1454BD5B, 0x013F00EE, 0x1E37D20F, 0x07473255,
     0x19959BA9, 0x01FAF16E, 0x0067875F}};

// Zero (constant)
static const int32_t ED25519_ZERO[9] = {0x00000000, 0x00000000, 0x00000000,
                                        0x00000000, 0x00000000, 0x00000000,
                                        0x00000000, 0x00000000, 0x00000000};

// Curve parameter d
static const int32_t ED25519_D[9] = {0x135978A3, 0x0F5A6E50, 0x10762ADD,
                                     0x00149A82, 0x1E898007, 0x003CBBBC,
                                     0x19CE331D, 0x1DC56DFF, 0x0052036C};

// Pre-computed value of 2 * d
static const int32_t ED25519_2D[9] = {0x06B2F159, 0x1EB4DCA1, 0x00EC55BA,
                                      0x00293505, 0x1D13000E, 0x00797779,
                                      0x139C663A, 0x1B8ADBFF, 0x002406D9};
...
__weak_func error_t ed25519GenerateSignature(
    const uint8_t *privateKey, const uint8_t *publicKey, const void *message,
  ...
  error = ed25519GenerateSignatureEx(privateKey, publicKey, messageChunks,
                                     arraysize(messageChunks), context,
...
error_t ed25519GenerateSignatureEx(const uint8_t *privateKey,
                                   ...
                                   const DataChunk *message, uint_t messageLen,
  ...
  for (i = 0; i < messageLen; i++) {
    ...
    sha512Update(&state->sha512Context, message[i].buffer, message[i].length);
  }
...
__weak_func error_t ed25519VerifySignature(const uint8_t *publicKey,
                                           ...
                                           size_t messageLen,
                                           const void *context,
                                           uint8_t contextLen, uint8_t flag,
...
error_t ed25519VerifySignatureEx(const uint8_t *publicKey,
                                 const DataChunk *message, uint_t messageLen,
                                 const void *context, uint8_t contextLen,
                                 uint8_t flag, const uint8_t *signature) {
  uint_t i;
  uint32_t ret;
#if (CRYPTO_STATIC_MEM_SUPPORT == DISABLED)
  ...
  ret = 1 ^
        ed25519SubInt(state->p, state->s, ED25519_L, ED25519_SIGNATURE_LEN / 2);
...
__weak_func void ed25519TwinMul(Ed25519SubState *state, Ed25519Point *r,
                                const uint8_t *k1, const Ed25519Point *p,
                                const uint8_t *k2, const Ed25519Point *q) {
  int_t i;
  uint8_t b1;
  uint8_t b2;
...
uint8_t ed25519SubInt(uint8_t *r, const uint8_t *a, const uint8_t *b,
                      uint_t n) {
  uint_t i;
  int16_t temp;
...
void ed25519MulInt(uint8_t *rl, uint8_t *rh, const uint8_t *a, const uint8_t *b,
                   uint_t n) {
  uint_t i;
  uint_t j;
  uint32_t temp;
...
void ed25519SelectInt(uint8_t *r, const uint8_t *a, const uint8_t *b, uint8_t c,
                      uint_t n) {
  uint_t i;
  uint8_t mask;
