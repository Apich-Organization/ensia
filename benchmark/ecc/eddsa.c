...
error_t eddsaGenerateKeyPair(const PrngAlgo *prngAlgo, void *prngContext,
  ...
  error = eddsaGeneratePrivateKey(prngAlgo, prngContext, curve, privateKey);
...
error_t eddsaGeneratePrivateKey(const PrngAlgo *prngAlgo, void *prngContext,
  ...
  if (curve != NULL && privateKey != NULL) {
    ...
    if (curve == ED25519_CURVE) {
      ...
      error = ed25519GeneratePrivateKey(prngAlgo, prngContext, privateKey->d);
    } else
#endif
      ...
      if (curve == ED448_CURVE) {
        ...
        error = ed448GeneratePrivateKey(prngAlgo, prngContext, privateKey->d);
      } else
#endif
      // Unknown algorithm?
      {
