...
__weak_func error_t ecGenerateKeyPair(const PrngAlgo *prngAlgo,
                                      void *prngContext, const EcCurve *curve,
...
error_t ecGeneratePublicKey(const EcPrivateKey *privateKey,
  ...
  // Compute Q = d.G
  error =
      ecMulRegular(publicKey->curve, &q, privateKey->d, &publicKey->curve->g);
  // Any error to report?
  if (error)
    return error;
    ...
    if (curve->type == EC_CURVE_TYPE_WEIERSTRASS ||
      ...
      if (format == EC_PUBLIC_KEY_FORMAT_X963) {
        ...
        if (length == (n * 2)) {
          ...
          error = ecScalarImport(key->q.x, EC_MAX_MODULUS_SIZE, input, n,
                                 EC_SCALAR_FORMAT_BIG_ENDIAN);

          // Check status code
          if (!error) {
            // Convert the y-coordinate to an integer
            error = ecScalarImport(key->q.y, EC_MAX_MODULUS_SIZE, input + n, n,
                                   EC_SCALAR_FORMAT_BIG_ENDIAN);
          }
        } else {
        ...
        // Convert the x-coordinate to an integer
        error = ecScalarImport(key->q.x, EC_MAX_MODULUS_SIZE, input, length,
                               EC_SCALAR_FORMAT_BIG_ENDIAN);
      } else if (format == EC_PUBLIC_KEY_FORMAT_RAW_Y) {
        // Convert the y-coordinate to an integer
        error = ecScalarImport(key->q.y, EC_MAX_MODULUS_SIZE, input, length,
                               EC_SCALAR_FORMAT_BIG_ENDIAN);
      } else {
        // Invalid format
...
__weak_func error_t ecMulFast(const EcCurve *curve, EcPoint3 *r,
  ...
  // Check whether Sz == 0
    ...
    for (i = pLen - 2; i >= 1; i--) {
      ...
      if (ecScalarGetBitValue(h, i) != 0 && ecScalarGetBitValue(k, i) == 0) {
...
__weak_func error_t ecTwinMul(const EcCurve *curve, EcPoint3 *r,
                              const uint32_t *d0, const EcPoint3 *s,
                              const uint32_t *d1, const EcPoint3 *t) {
  int_t k;
  uint_t pLen;
  uint_t m;
  uint_t m0;
  ...
  for (k = m; k >= 0; k--) {
    // Compute h(0) = 16 * c(0,1) + 8 * c(0,2) + 4 * c(0,3) + 2 * c(0,4) +
    // c(0,5)
    h0 = c0 & 0x1F;
    ...
    // Compute h(1) = 16 * c(1,1) + 8 * c(1,2) + 4 * c(1,3) + 2 * c(1,4) +
    // c(1,5)
    h1 = c1 & 0x1F;
