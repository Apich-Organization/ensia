...
const uint8_t DSA_WITH_SHA224_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                        0x03, 0x04, 0x03, 0x01};
// DSA with SHA-256 OID (2.16.840.1.101.3.4.3.2)
const uint8_t DSA_WITH_SHA256_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                        0x03, 0x04, 0x03, 0x02};
// DSA with SHA-384 OID (2.16.840.1.101.3.4.3.3)
const uint8_t DSA_WITH_SHA384_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                        0x03, 0x04, 0x03, 0x03};
// DSA with SHA-512 OID (2.16.840.1.101.3.4.3.4)
const uint8_t DSA_WITH_SHA512_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                        0x03, 0x04, 0x03, 0x04};
// DSA with SHA-3-224 OID (2.16.840.1.101.3.4.3.5)
const uint8_t DSA_WITH_SHA3_224_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                          0x03, 0x04, 0x03, 0x05};
// DSA with SHA-3-256 OID (2.16.840.1.101.3.4.3.6)
const uint8_t DSA_WITH_SHA3_256_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                          0x03, 0x04, 0x03, 0x06};
// DSA with SHA-3-384 OID (2.16.840.1.101.3.4.3.7)
const uint8_t DSA_WITH_SHA3_384_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                          0x03, 0x04, 0x03, 0x07};
// DSA with SHA-3-512 OID (2.16.840.1.101.3.4.3.8)
const uint8_t DSA_WITH_SHA3_512_OID[9] = {0x60, 0x86, 0x48, 0x01, 0x65,
                                          0x03, 0x04, 0x03, 0x08};
...
error_t dsaImportSignature(DsaSignature *signature, const uint8_t *input,
  ...
  do {
    ...
    error =
        mpiImport(&signature->r, tag.value, tag.length, MPI_FORMAT_BIG_ENDIAN);
    // Any error to report?
    if (error)
      break;
    ...
    // Convert the octet string to a multiple precision integer
    error =
        mpiImport(&signature->s, tag.value, tag.length, MPI_FORMAT_BIG_ENDIAN);
    // Any error to report?
    if (error)
      break;
...
error_t dsaGenerateSignature(const PrngAlgo *prngAlgo, void *prngContext,
  ...
  while (1) {
    ...
    MPI_CHECK(
        mpiExpModRegular(&signature->r, &key->params.g, &k, &key->params.p));
