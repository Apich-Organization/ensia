...
// S-box S
#define S(x)                                                                   \
  ((uint32_t)s1[(x) & 0xFF] | ((uint32_t)s0[((x) >> 8) & 0xFF] << 8) |         \
...
// Key loading (ZUC-128)
#define LOAD1(a, b, c)                                                         \
  (((uint32_t)(a) << 23) | ((uint32_t)(b) << 8) | (uint32_t)(c))

// Key loading (ZUC-256)
#define LOAD2(a, b, c, d)                                                      \
  (((uint32_t)(a) << 23) | ((uint32_t)((b) & 0x7F) << 16) |                    \
   ((uint32_t)(c) << 8) | (uint32_t)(d))

// Constant D (ZUC-128)
static const uint16_t d1[16] = {0x44D7, 0x26BC, 0x626B, 0x135E, 0x5789, 0x35E2,
                                0x7135, 0x09AF, 0x4D78, 0x2F13, 0x6BC4, 0x1AF1,
                                0x5E26, 0x3C4D, 0x789A, 0x47AC};

// Constant D (ZUC-256 with 128-bit IV)
static const uint8_t d2[16] = {0x64, 0x43, 0x7B, 0x2A, 0x11, 0x05, 0x51, 0x42,
                               0x1A, 0x31, 0x18, 0x66, 0x14, 0x2E, 0x01, 0x5C};

// Constant D (ZUC-256 with 184-bit IV)
static const uint8_t d3[16] = {0x22, 0x2F, 0x24, 0x2A, 0x6D, 0x40, 0x40, 0x40,
...
error_t zucInit(ZucContext *context, const uint8_t *key, size_t keyLen,
  ...
  if (keyLen == 16 && ivLen == 16) {
    ...
    context->s[14] =
        LOAD2(key[14], d3[14] | ((key[31] >> 4) & 0x0F), iv[16], iv[9]);
    context->s[15] =
        LOAD2(key[15], d3[15] | (key[31] & 0x0F), key[30], key[29]);
  } else {
    // Report an error
    return ERROR_INVALID_PARAMETER;
  }
