  ...
  if (format == MPI_FORMAT_LITTLE_ENDIAN) {
    ...
    if (!error) {
      ...
      for (i = 0; i < length; i++, input++) {
        temp = *input & 0xFF;
        r->data[i / MPI_BYTES_PER_WORD] |= temp
                                           << ((i % MPI_BYTES_PER_WORD) * 8);
      }
    }
  ...
  if (format == MPI_FORMAT_LITTLE_ENDIAN) {
    ...
    if (n <= length) {
      ...
      for (i = 0; i < n; i++, output++) {
        temp =
            a->data[i / MPI_BYTES_PER_WORD] >> ((i % MPI_BYTES_PER_WORD) * 8);
        *output = temp & 0xFF;
      }
    } else {
      // Report an error
...
error_t mpiShiftLeft(Mpi *r, uint_t n) {
  ...
  if (n2 > 0) {
    ...
    for (i = r->size - 1; i >= 1; i--) {
      r->data[i] =
          (r->data[i] << n2) | (r->data[i - 1] >> (MPI_BITS_PER_WORD - n2));
    }
...
__weak_func error_t mpiMulMod(Mpi *r, const Mpi *a, const Mpi *b,
                              const Mpi *p) {
  error_t error;
...
__weak_func error_t mpiExpMod(Mpi *r, const Mpi *a, const Mpi *e,
                              const Mpi *p) {
  error_t error;
  int_t i;
  int_t j;
...
__weak_func error_t mpiExpModFast(Mpi *r, const Mpi *a, const Mpi *e,
                                  const Mpi *p) {
...
__weak_func error_t mpiExpModRegular(Mpi *r, const Mpi *a, const Mpi *e,
                                     const Mpi *p) {
  // Perform modular exponentiation
  return mpiExpMod(r, a, e, p);
}
