...
#define ROUND(a, b, c, x, mul)                                                 \
  {                                                                            \
    c ^= x;                                                                    \
    a -= t1[c & 0xFF] ^ t2[(c >> 16) & 0xFF] ^ t3[(c >> 32) & 0xFF] ^          \
         t4[(c >> 48) & 0xFF];                                                 \
    b += t4[(c >> 8) & 0xFF] ^ t3[(c >> 24) & 0xFF] ^ t2[(c >> 40) & 0xFF] ^   \
         t1[(c >> 56) & 0xFF];                                                 \
    b *= mul;                                                                  \
  }
...
const uint8_t TIGER_OID[9] = {0x2B, 0x06, 0x01, 0x04, 0x01,
                              0xDA, 0x47, 0x0C, 0x02};

// Common interface for hash algorithms
const HashAlgo tigerHashAlgo = {"Tiger",
                                TIGER_OID,
                                sizeof(TIGER_OID),
                                sizeof(TigerContext),
                                TIGER_BLOCK_SIZE,
