...
4. **AntiDebugging:** Multi-vector hardware and kernel probes: direct raw syscall `/proc/self/status` TracerPid continuous auditing (immune to `catch syscall ptrace`), kernel anti-attachment via `prctl(PR_SET_DUMPABLE, 0)`, hardware debug registers (DR0-DR7), distributed in-flight checks (local `EFLAGS.TF` & local RDTSC jitter), priority 0 constructor watchdog, arithmetic data-flow entanglement, and unrecoverable violent exit.
