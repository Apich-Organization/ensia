...
static void
orViaXorAnd(BinaryOperator *bo); // a|b = (a^b)^(a&b) — wrong! Use (a^b)|(a&b)
static void
orNotNor(BinaryOperator *bo);            // a|b = NOR(NOR(a,b),NOR(a,b)) variant
...
static void
mulDoubleHalf(BinaryOperator *bo); // a*b = 2*(a*(b/2)) for even b (const only)

static void (*funcAdd[NUMBER_ADD_SUBST])(BinaryOperator *bo) = {
    &addNeg,           &addDoubleNeg,    &addRand,
    &addRand2,         &addSubstitution, &addSubstitution2,
    &addSubstitution3, &addChainedMBA,   &addRotateDecompose,
    &addNegComplement, &addRandPair,     &addFourLayerChain,
    &addNegateNegate};
static void (*funcSub[NUMBER_SUB_SUBST])(BinaryOperator *bo) = {
    &subNeg,           &subRand,          &subRand2,         &subSubstitution,
    &subSubstitution2, &subSubstitution3, &subViaComplement, &subFourTerm,
    &subDoubleNeg,     &subRandPair};
static void (*funcAnd[NUMBER_AND_SUBST])(BinaryOperator *bo) = {
    &andSubstitution,  &andSubstitution2,
    &andSubstitution3, &andSubstitutionRand,
    &andNor,           &andNand,
    &andViaXorOr,      &andNotNot,
    &andFourTerm,      &andMirror};
static void (*funcOr[NUMBER_OR_SUBST])(BinaryOperator *bo) = {
    &orSubstitution, &orSubstitution2, &orSubstitution3, &orSubstitutionRand,
    &orNor,          &orNand,          &orViaXorAnd,     &orNotNor,
    &orMerge,        &orFourTerm};
static void (*funcXor[NUMBER_XOR_SUBST])(BinaryOperator *bo) = {
    &xorSubstitution,  &xorSubstitution2,
    &xorSubstitution3, &xorSubstitutionRand,
    &xorNor,           &xorNand,
    &xorSplitRotate,   &xorArithDecompose,
    &xorViaCompl,      &xorHighLow,
    &xorDoubleRand,    &xorFourTerm};
...
static void shlSubstituteMul(BinaryOperator *bo) {
  ...
  BinaryOperator *mul =
      BinaryOperator::Create(Instruction::Mul, bo->getOperand(0), mulC, "", bo);
...
static void ashrSubstituteDoubleRound(BinaryOperator *bo) {
  ...
  // ^ (r1 >>s k) ^ (r2 >>s k)
  BinaryOperator *xR1 =
      BinaryOperator::Create(Instruction::Xor, ashrOp, r1Shr, "", bo);
  bo->replaceAllUsesWith(
...
static void mulSubstitution3(BinaryOperator *bo) {
  ...
  if (width < 4) {
    ...
    op = BinaryOperator::Create(
        Instruction::Sub,
        BinaryOperator::Create(Instruction::Mul, bo->getOperand(0),
                               BinaryOperator::CreateNeg(co, "", bo), "", bo),
        BinaryOperator::Create(Instruction::Mul, op, co, "", bo), "", bo);
    bo->replaceAllUsesWith(op);
    return;
  }
  unsigned k = width / 2;
  ConstantInt *kConst = cast<ConstantInt>(ConstantInt::get(bo->getType(), k));
  ConstantInt *maskConst = cast<ConstantInt>(
      ConstantInt::get(bo->getType(), APInt::getLowBitsSet(width, k)));
...
static void addChainedMBA(BinaryOperator *bo) {
  ...
  BinaryOperator *dbl =
      BinaryOperator::Create(Instruction::Mul, sor, c2, "", bo);
  bo->replaceAllUsesWith(
...
static void addRotateDecompose(BinaryOperator *bo) {
  ...
  BinaryOperator *andr_a =
      BinaryOperator::Create(Instruction::And, a, r, "", bo);
  BinaryOperator *andr_b =
      BinaryOperator::Create(Instruction::And, b, r, "", bo);
  BinaryOperator *twoa =
      BinaryOperator::Create(Instruction::Mul, andr_a, c2, "", bo);
  BinaryOperator *twob =
      BinaryOperator::Create(Instruction::Mul, andr_b, c2, "", bo);
  BinaryOperator *twoR =
...
static void xorSplitRotate(BinaryOperator *bo) {
  ...
  BinaryOperator *andNB =
      BinaryOperator::Create(Instruction::And, notA, b, "", bo);
  BinaryOperator *dbl =
      BinaryOperator::Create(Instruction::Mul, andNB, c2, "", bo);
...
// XOR arithmetic decompose: a^b = (a+b) - 2*(a&b), then expand (a&b) via De
// Morgan Result: a^b = (a+b) - 2*(~(~a | ~b)) Two negations in the expression
// chain defeat HLIL's and-pattern recognition.
static void xorArithDecompose(BinaryOperator *bo) {
  Value *a = bo->getOperand(0), *b = bo->getOperand(1);
  Type *T = bo->getType();
  ConstantInt *c2 = (ConstantInt *)ConstantInt::get(T, 2);
  ...
  BinaryOperator *andAB =
      BinaryOperator::CreateNot(orN, "", bo); // ~(~a|~b) = a&b
  BinaryOperator *dbl =
...
// decomposition ~a = (a XOR r) XOR ~r  for any constant r (XOR with r then XOR
// with ~r = NOT)
...
static void andFourTerm(BinaryOperator *bo) {
  ...
  BinaryOperator *anr =
      BinaryOperator::Create(Instruction::And, a, notR, "", bo); // a & ~r
  BinaryOperator *br =
      BinaryOperator::Create(Instruction::And, b, r, "", bo); // b & r
  BinaryOperator *bnr =
...
static void orNotNor(BinaryOperator *bo) {
  Value *a = bo->getOperand(0), *b = bo->getOperand(1);
  // NOR(a,b) = ~(a|b); NOR(NOR(a,b), NOR(a,b)) = ~(~(a|b) | ~(a|b)) = ~~(a|b) =
  // a|b ✓
